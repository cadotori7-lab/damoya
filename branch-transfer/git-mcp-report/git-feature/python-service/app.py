...
async def lifespan(app: FastAPI):
    ...
    await app.state.git_report_service.start()
    yield
    await app.state.chat_service.close()
    await app.state.mentor_service.close()
    await app.state.git_report_service.close()
...
app = FastAPI(
    ...
    description=(
        "Spring MVC 웹 화면과 MCP 사이트 검색, 멘토 추천, "
        "Git 프로젝트 리포트 서버를 중계합니다."
    ),
    lifespan=lifespan,
)
