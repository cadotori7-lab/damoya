...
# Git 프로젝트 AI 리포트 MCP 설정
GIT_REPORT_ALLOWED_HOSTS = {
    host.strip().lower()
    for host in os.getenv("GIT_REPORT_ALLOWED_HOSTS", "github.com").split(",")
    if host.strip()
}
GIT_REPORT_GIT_EXECUTABLE = os.getenv("GIT_REPORT_GIT_EXECUTABLE", "git").strip()
GIT_REPORT_CLONE_DEPTH = int(os.getenv("GIT_REPORT_CLONE_DEPTH", "50"))
GIT_REPORT_CLONE_TIMEOUT_SECONDS = int(
    os.getenv("GIT_REPORT_CLONE_TIMEOUT_SECONDS", "90")
)
GIT_REPORT_COMMAND_TIMEOUT_SECONDS = int(
    os.getenv("GIT_REPORT_COMMAND_TIMEOUT_SECONDS", "30")
)
GIT_REPORT_MAX_REPOSITORY_BYTES = int(
    os.getenv("GIT_REPORT_MAX_REPOSITORY_BYTES", str(250 * 1024 * 1024))
)
GIT_REPORT_MAX_TRACKED_FILES = int(
    os.getenv("GIT_REPORT_MAX_TRACKED_FILES", "10000")
)
GIT_REPORT_MAX_FILE_BYTES = int(
    os.getenv("GIT_REPORT_MAX_FILE_BYTES", str(512 * 1024))
)
GIT_REPORT_MAX_FILES_PER_READ = int(
    os.getenv("GIT_REPORT_MAX_FILES_PER_READ", "20")
)
GIT_REPORT_MAX_RETURNED_CHARS = int(
    os.getenv("GIT_REPORT_MAX_RETURNED_CHARS", "400000")
)
GIT_REPORT_WORKSPACE_TTL_SECONDS = int(
    os.getenv("GIT_REPORT_WORKSPACE_TTL_SECONDS", "3600")
)
GIT_REPORT_WORK_DIR = Path(
    os.getenv(
        "GIT_REPORT_WORK_DIR",
        str(BASE_DIR.parent / "tmp" / "git-report"),
    )
)
...
GIT_REPORT_PDF_TEMP_DIR = Path(
    os.getenv(
        "GIT_REPORT_PDF_TEMP_DIR",
        str(BASE_DIR.parent / "tmp" / "pdfs"),
    )
)
GIT_REPORT_PLAYWRIGHT_CHANNEL = os.getenv(
    "GIT_REPORT_PLAYWRIGHT_CHANNEL",
    "",
).strip()
GIT_REPORT_PDF_FONT_FAMILY = os.getenv(
    "GIT_REPORT_PDF_FONT_FAMILY",
    (
